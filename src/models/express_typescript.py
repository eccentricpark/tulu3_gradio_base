import gradio as gr
import ollama
from src.utils.utils import get_file_name_for_save, get_css, getPromptByJson
...
def ai_response(user_input, chat_history):
    """
    1) chat_history에 (user_input, "") 추가
    2) Textbox를 즉시 ""로 초기화 -> yield
    3) Ollama에서 partial response를 받아 chat_history[-1] (assistant 답변)을 점진적으로 업데이트 -> yield
    4) 최종적으로 messageList에도 assistant 답변을 저장
    """
    ...
    response = ollama.chat(
        model=model_name,
        messages=[getPromptByJson("express_typescript"), *messageList],
        stream=True
    )
...
def save_conversation_as_md(chat_history):
    """
    현재까지의 chat_history를 .md 파일로 저장
    (chat_history: [(user_input, assistant_reply), ...] 형태)
    """
    if not chat_history:
        return "대화 내역이 없습니다."

    # 타임스탬프를 이용해 고유한 파일명 생성 (예: conversation_2025_01_31_10_22_35.md)
    filename = get_file_name_for_save(name="express_typescript")
...
def get_model_by_prompt_express_typescript():
    """
    Gradio Blocks로 Chatbot + Textbox 구성 + 파일로 저장 기능 추가
    """
    with gr.Blocks(css=get_css()) as demo:
        gr.Markdown("## Tulu3 8B (express typescript)")
